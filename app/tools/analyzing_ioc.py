...
def _load_project_model(path, modified_ns):
    del modified_ns
    return STM32CubeMXParser(path).parse(Path(path).parent)


class _legacy_analyzing_ioc:
    @staticmethod
...
class analyzing_ioc(_legacy_analyzing_ioc):
    """Compatibility facade backed by MCode's single CubeMX parser.

    The Qt pages keep their existing return shapes during migration, while all
    project discovery now comes from the same ProjectModel used by the CLI.
    """

    @staticmethod
    def _model(ioc_path):
        path = Path(ioc_path).resolve()
        return _load_project_model(str(path), path.stat().st_mtime_ns)

    @staticmethod
    def _names(ioc_path, *kinds):
        return sorted(item.name for item in analyzing_ioc._model(ioc_path).peripherals_of(*kinds))

    @staticmethod
    def is_freertos_enabled_from_ioc(ioc_path):
        return analyzing_ioc._model(ioc_path).freertos

    @staticmethod
    def get_enabled_i2c_from_ioc(ioc_path):
        return analyzing_ioc._names(ioc_path, "I2C")

    @staticmethod
    def get_enabled_spi_from_ioc(ioc_path):
        return analyzing_ioc._names(ioc_path, "SPI")

    @staticmethod
    def get_enabled_can_from_ioc(ioc_path):
        return analyzing_ioc._names(ioc_path, "CAN")

    @staticmethod
    def get_enabled_fdcan_from_ioc(ioc_path):
        return analyzing_ioc._names(ioc_path, "FDCAN")

    @staticmethod
    def get_enabled_uart_from_ioc(ioc_path):
        return analyzing_ioc._names(ioc_path, "UART", "USART", "LPUART")

    @staticmethod
    def get_enabled_gpio_from_ioc(ioc_path):
        return [
            {"pin": pin.name, "label": pin.label}
            for pin in analyzing_ioc._model(ioc_path).pins
            if pin.label and pin.signal.startswith("GPXTI")
        ]
    ...
    def get_all_gpio_from_ioc(ioc_path):
        result = []
        for pin in analyzing_ioc._model(ioc_path).pins:
            if not pin.label or not (pin.signal in {"GPIO_Input", "GPIO_Output"} or pin.signal.startswith("GPXTI")):
                continue
            result.append({
                "pin": pin.name,
                "label": pin.label,
                "has_exti": "GPIO_ModeDefaultEXTI" in pin.attributes or pin.signal.startswith("GPXTI"),
                "signal": pin.signal,
                "mode": pin.attributes.get("GPIO_ModeDefaultEXTI", ""),
                "is_output": pin.signal == "GPIO_Output",
                "is_input": pin.signal == "GPIO_Input",
            })
        return result
    ...
    def get_enabled_pwm_from_ioc(ioc_path):
        result = []
        for pin in analyzing_ioc._model(ioc_path).pins:
            match = re.match(r"S_(TIM\d+)_CH(\d+)", pin.signal)
            if not match:
                continue
            timer, channel = match.groups()
            result.append({
                "timer": timer,
                "channel": f"TIM_CHANNEL_{channel}",
                "label": pin.label or f"{timer}_CH{channel}",
                "pin": pin.name,
                "signal": pin.signal,
            })
        return result
    ...
    @staticmethod
    def get_mcu_name_from_ioc(ioc_path):
        return analyzing_ioc._model(ioc_path).mcu or None
