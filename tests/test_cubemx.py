...
class CubeMXParserTests(unittest.TestCase):
    def test_parses_real_f4_project_once_into_model(self):
        path = REPOSITORY / "assets/User_code/ioc/DevC.ioc"
    ...
    def test_parses_real_h7_fdcan_project(self):
        path = REPOSITORY / "assets/User_code/ioc/CtrBoard-H7_ALL.ioc"
        model = STM32CubeMXParser(path).parse()
    ...
    def test_legacy_gui_facade_uses_same_model(self):
        path = REPOSITORY / "assets/User_code/ioc/DevC.ioc"
        self.assertEqual(analyzing_ioc.get_enabled_can_from_ioc(path), ["CAN1", "CAN2"])
        self.assertEqual(analyzing_ioc.get_mcu_name_from_ioc(path), STM32CubeMXParser(path).parse().mcu)
        self.assertTrue(analyzing_ioc.get_enabled_gpio_from_ioc(path))
        self.assertTrue(analyzing_ioc.get_enabled_pwm_from_ioc(path))
